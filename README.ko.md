...
### 배포 / 설치
개발자는 **vouched(승인된)** 기능만 가져옵니다 — 라이브 upstream이 아니라. 콘솔에서 Skill은 박제된 파일로, MCP 서버는 vouched 연결 설정으로 설치됩니다(*"Add to Claude / Add to Codex"*). Vouchq는 신뢰된 아티팩트를 발급할 뿐, 요청 경로에 인라인으로 끼어들지 않습니다.
...
```
  ...
  └─────────────────────────────────────────────────────────────────────────┘
                              │  승인·박제된 것만
  ┌── 배포 (나가는 문) ───────▼─────────────────────────────────────────────┐
  │   Add to Claude / Add to Codex   (박제 Skill 파일 · vouched MCP 설정)    │
  └─────────────────────────────────────────────────────────────────────────┘
```
