...
### 자가 거버넌스 룰셋 (Sentinel)
Vouchq는 파는 기준을 자신에게도 적용합니다. 스캐너가 오픈소스이므로 그 룰셋 자체가 공급망 표적입니다 — 룰을 슬쩍 약화하는 PR이나 변조된 빌드면 악성 정의가 통과해 버립니다. 그래서 Vouchq는 봉인된 **카나리 코퍼스**(CRITICAL 룰마다 알려진 악성 픽스처 1개)를 동봉하고, 자기 스캐너를 그 코퍼스에 **기동 시·매시간·CI 게이트**로 계속 돌립니다. 카나리가 하나라도 미탐지되면 룰셋이 약화된 것: Vouchq는 **DEGRADED** 로 전환해 **fail-closed**(승인 발급 중단 — 자기가 보증 못 할 신뢰는 안 만듦)하고, WORM 감사에 기록하며, `GET /api/ruleset/health` 로 상태+룰셋 지문을 노출합니다. 룰을 약화하면 카나리가 어두워져 반드시 들킵니다.
...
2. **리뷰 & 승인** — 기능을 열어 findings와 정의를 보고 **Approve & pin**. Vouchq가 바로 그 바이트를 불변 기준선(정본)으로 박제합니다.
3. **계속 감시** — 재스캔이 라이브 정의를 박제본과 비교해, 바뀌면 **드리프트** 경보(=rug-pull 알람)를 띄웁니다.
